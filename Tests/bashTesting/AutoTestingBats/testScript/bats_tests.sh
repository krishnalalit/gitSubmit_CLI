...
# This checks that there is a file called "MainModuleTestVariant.sh"
# in this directory. If this fails, it's because you haven't
# created the file, or you created it in the wrong place.
@test "The shell script file 'MainModuleTestVariant.sh' exists" {
  [ -f "MainModuleTestVariant.sh" ]
}

# This checks that the a file called "MainModuleTestVariant.sh"
...
@test "The shell script file 'MainModuleTestVariant.sh' is executable" {
  [ -x "MainModuleTestVariant.sh" ]
}

# This checks that running the script doesn't generate an error code.
@test "The script runs without generating an error code" {
  run ./MainModuleTestVariant.sh test_data/files.tgz
  [ "$status" -eq 0 ]
}
